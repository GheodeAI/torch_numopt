...
def create_line_search_solver(method, condition, c1=1e-4, c2=0.9, tau=0.1):
    match method:
        case "backtrack":
            line_search = BacktrackingLineSearch(condition=condition, c1=c1, c2=c2, tau=tau)
        case "interpolate":
            line_search = InterpolationLineSearch(condition=condition, c1=c1, c2=c2, tau=tau)
        case "bisect":
            line_search = BisectionLineSearch(condition=condition, c1=c1, c2=c2, tau=tau)
        case _:
...
class BisectionLineSearch(LineSearchSolver):
    ...
    def line_search(self, params, step_dir, d_p_list, lr_init, eval_model):
        ...
        for _ in range(self.iter_max):
            if torch.abs(new_dir_deriv) < self.tol or a_max == a_min:
                break
