...
class SecondOrderOptimizer(LineSearchOptimizer, ABC):
    ...
    def exact_hessian(self, x, y, loss_fn, vectorize=True):
        ...
        if self.batch_size is None or self.batch_size >= len(x):
            ...
            h_list = list(torch.func.hessian(eval_model, argnums=tuple(range(len(model_params))))(*model_params))
            ...
            for start in batch_start:
                ...
                h_list_batch = list(torch.func.hessian(eval_model, argnums=tuple(range(len(model_params))))(*model_params))
    ...
    def approx_hessian_gn(self, x, y, loss_fn, vectorize=True):
        ...
        if self.batch_size is None or self.batch_size >= len(x):
            ...
            j_list = torch.autograd.functional.jacobian(
                get_residuals, model_params, create_graph=False, vectorize=vectorize
            )
            ...
            for start in batch_start:
                ...
                if h_list is None:
                    ...
                    h_list = [
                        batch_h + prev_h
                        for batch_h, prev_h in zip(h_list, h_list_batch)
                    ]
    ...
    def hutchinson_diagonal(
        ...
        for _ in range(n_samples):
            ...
            z_flat = (
                2
                * torch.bernoulli(
                    torch.full_like(params_flat, 0.5, device=params_flat.device)
                )
                - 1
            )
            ...
            _, Hz = torch.autograd.functional.vhp(
                eval_model, model_params, v=z, create_graph=False
            )
    ...
    def _reshape_hessian(hess: torch.Tensor):
        ...
        ), "Hessian an the incorrect shape."
